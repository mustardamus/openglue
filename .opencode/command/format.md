...
| `Dockerfile`, `Containerfile`        | dockerfile-utils | `npm:dockerfile-utils` | see note below                |

> **Note:** `dockerfile-utils format` writes to stdout only (no in-place flag).
> Write back in place with a subshell:
> `out=$(dockerfile-utils format <file>) && printf '%s\n' "$out" > <file>`
